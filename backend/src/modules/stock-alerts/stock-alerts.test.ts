...
test("activa la alerta con 0 disponibles y mínimo 0", () => {
  ...
  assert.equal(stockAlertMessage("Smartphone", 0, 0), "No quedan Smartphones disponibles");
});

test("activa la alerta con 3 disponibles y mínimo 3", () => {
  assert.equal(stockAlertIsTriggered(3, 3, true), true);
  assert.match(stockAlertMessage("Notebook", 3, 3), /Quedan 3 Notebooks disponibles/);
});
