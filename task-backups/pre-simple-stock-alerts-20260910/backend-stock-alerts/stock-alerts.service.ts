...
export const stockAlertMessage = (
  ...
  : `${disponibles === 1 ? "Queda 1 disponible" : `Quedan ${disponibles} disponibles`} · mínimo configurado: ${minimoDisponible}`;
...
const mapStockAlert = (row: StockAlertRow): StockAlertConfiguration => {
  ...
  return {
    ...
    enAlerta,
    mensaje: enAlerta ? stockAlertMessage(disponibles, row.minimo_disponible) : null,
