...
@Component({
  ...
  imports: [DatePipe, RutPipe, LucideDroplet, LucidePrinter, LucideX],
  template: `
    <div class="dialog-backdrop document-backdrop" role="presentation" (click)="onBackdrop($event)">
      <article class="dialog dialog--wide print-document" role="dialog" aria-modal="true" [attr.aria-labelledby]="documentType() === 'receipt' ? 'receipt-title' : 'delivery-title'">
        ...
        <section class="document-section"><h3>Funcionario responsable</h3><dl><div><dt>Nombre</dt><dd>{{ collaborator()?.nombre || '—' }}</dd></div><div><dt>RUT</dt><dd>{{ (collaborator()?.rut | rut) || '—' }}</dd></div><div><dt>Cargo</dt><dd>{{ collaborator()?.cargo || '—' }}</dd></div><div><dt>Departamento</dt><dd>{{ departmentName() || device().departamento?.nombre || '—' }}</dd></div></dl></section>
