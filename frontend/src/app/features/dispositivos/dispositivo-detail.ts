        ...
        <aside class="panoramic-column actions-column">
          ...
          @if (!action()) {
            <div class="operation-list">
              <button type="button" [disabled]="terminal() || device.tipoCustodia!=='NONE'" (click)="open('assign-person')"><svg lucideUserCheck></svg><span>Entregar equipo<small>A una persona habilitada</small></span></button>
              @if((device.colaborador || device.departamento) && !terminal()){<button type="button" class="operation-return" (click)="open('return')"><svg lucideRotateCcw></svg><span>Recibir equipo<small>Registra recepción y revisión</small></span></button>}
              <button type="button" [disabled]="terminal()" (click)="open('service')"><svg lucideWrench></svg><span>Revisión técnica<small>Crear orden y bloquear movimientos</small></span></button>
              ...
              <button type="button" (click)="open('assign-department')" [disabled]="terminal() || device.tipoCustodia!=='NONE'"><svg lucideBuilding></svg><span>Entregar al departamento<small>Responsabilidad institucional</small></span></button>
            ...
            <form class="action-form" [formGroup]="actionForm" (ngSubmit)="execute()">
              ...
              @if(action()==='assign-person'){<div class="field"><label for="colaborador">Persona responsable *</label><select id="colaborador" formControlName="colaboradorId"><option value="">Seleccionar persona</option>@for(person of collaborators(); track person.id){<option [value]="person.id">{{ person.nombre }} · {{ person.rut }}</option>}</select></div>}
              @if(action()==='assign-department'){<div class="notice notice--info">La responsabilidad será institucional; la persona que recibe confirma la entrega física.</div><div class="field"><label for="dept">Departamento *</label><select id="dept" formControlName="departamentoId"><option value="">Seleccionar</option>@for(department of departments(); track department.id){<option [value]="department.id">{{department.nombre}}</option>}</select></div><div class="field"><label for="receiver">Persona que recibe *</label><select id="receiver" formControlName="recibidoPorId"><option value="">Seleccionar persona del departamento</option>@for(person of departmentReceivers();track person.id){<option [value]="person.id">{{person.nombre}} · {{person.rut}} · {{person.cargo||'Sin cargo'}}</option>}</select></div><div class="field"><label for="location">Localidad</label><input id="location" formControlName="localidad" maxlength="120" /></div><div class="field"><label for="position">Ubicación</label><input id="position" formControlName="ubicacionDetalle" maxlength="250" /></div>}
...
export class DispositivoDetail implements OnInit {
  ...
  protected actionTitle(): string { return { 'assign-person': 'Registrar entrega', 'assign-department': 'Entregar al departamento', return: 'Registrar recepción', state: 'Cambiar estado', recover: 'Registrar equipo encontrado', service:'Enviar a revisión técnica',retire:'Retirar del inventario' }[this.action() || 'return']; }
