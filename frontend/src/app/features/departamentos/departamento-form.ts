...
export class DepartamentoForm implements OnInit {
 ...
 protected readonly form=this.fb.nonNullable.group({nombre:['',[Validators.required,Validators.maxLength(120)]],observaciones:[''],activo:[true]});
 ngOnInit(){this.id=this.route.snapshot.paramMap.get('id')||'';if(this.id){this.loading.set(true);this.service.obtener(this.id).subscribe({next:item=>{this.form.patchValue({nombre:item.nombre,observaciones:item.observaciones||'',activo:item.activo});this.loading.set(false)},error:e=>{this.apiError.set(errorMessage(e));this.loading.set(false)}})}}
 protected invalid(name:'nombre'){const c=this.form.controls[name];return c.invalid&&(c.dirty||c.touched)}
 protected submit(){if(this.form.invalid){this.form.markAllAsTouched();return}this.submitting.set(true);this.apiError.set('');const raw=this.form.getRawValue();const input={...raw,observaciones:raw.observaciones.trim()||null};const request=this.id?this.service.actualizar(this.id,input):this.service.crear(input);request.subscribe({next:()=>this.router.navigate(['/departamentos']),error:e=>{this.apiError.set(errorMessage(e));this.submitting.set(false)}})}
}
