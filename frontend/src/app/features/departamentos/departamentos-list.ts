...
@Component({ selector:'app-departamentos-list', imports:[FormsModule,RouterLink,PageHeader,StatusBadge,ViewState], template:`
  ...
  <section class="card"><div class="toolbar"><div class="field"><label for="search">Buscar</label><input id="search" [(ngModel)]="query" placeholder="Departamento o dependencia" /></div><div class="field"><label for="status">Estado</label><select id="status" [(ngModel)]="status"><option value="">Todos</option><option value="active">Activos</option><option value="inactive">Inactivos</option></select></div></div>
  ...
  <div class="table-wrap"><table class="data-table"><thead><tr><th>Departamento</th><th>Dependencia</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>@for(item of filtered();track item.id){<tr><td class="cell-primary">{{item.nombre}}</td><td>{{item.dependencia_nombre||'Nivel principal'}}</td><td><app-status-badge [code]="item.activo" [label]="item.activo?'Activo':'Inactivo'" /></td><td><div class="actions"><a class="btn btn--secondary btn--small" [routerLink]="[item.id]">Ver activos</a><a class="btn btn--ghost btn--small" [routerLink]="[item.id,'editar']">Editar</a>@if(item.activo){<button class="btn btn--danger btn--small" type="button" (click)="deactivate(item)">Desactivar</button>}</div></td></tr>}</tbody></table></div>}
  </section>` })
export class DepartamentosList implements OnInit {
  ...
  protected filtered(){const q=this.query.toLowerCase().trim();return this.items().filter(i=>(!q||i.nombre.toLowerCase().includes(q)||(i.dependencia_nombre||'').toLowerCase().includes(q))&&(!this.status||(this.status==='active')===i.activo))}
