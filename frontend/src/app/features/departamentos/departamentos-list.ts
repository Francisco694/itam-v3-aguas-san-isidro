...
@Component({ selector:'app-departamentos-list', imports:[DatePipe,FormsModule,RouterLink,PageHeader,StatusBadge,ViewState], template:`
  ...
  <div class="table-wrap"><table class="data-table"><thead><tr><th>Departamento</th><th>Dependencia</th><th>Observaciones</th><th>Estado</th><th>Actualización</th><th></th></tr></thead><tbody>@for(item of filtered();track item.id){<tr><td class="cell-primary">{{item.nombre}}</td><td>{{item.dependencia_nombre||'Nivel principal'}}</td><td class="text-muted">{{item.observaciones||'Sin observaciones'}}</td><td><app-status-badge [code]="item.activo" [label]="item.activo?'Activo':'Inactivo'" /></td><td><span class="cell-secondary">{{item.actualizadoEn|date:'dd-MM-yyyy HH:mm'}}</span></td><td><div class="actions"><a class="btn btn--secondary btn--small" [routerLink]="[item.id]">Ver activos</a><a class="btn btn--ghost btn--small" [routerLink]="[item.id,'editar']">Editar</a>@if(item.activo){<button class="btn btn--danger btn--small" type="button" (click)="deactivate(item)">Desactivar</button>}</div></td></tr>}</tbody></table></div>}
  </section>` })
