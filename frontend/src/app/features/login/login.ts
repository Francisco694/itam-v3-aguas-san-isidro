    ...
    <main>
      <section class="login-card">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()">
          @if (sessionMessage) {
            <div class="notice notice--info">{{ sessionMessage }}</div>
          }
          @if (error()) {
  ...
  protected readonly sessionMessage =
    this.route.snapshot.queryParamMap.get('reason') === 'idle'
      ...
      : this.route.snapshot.queryParamMap.get('reason') === 'session'
        ? 'Tu sesión ya no es válida. Vuelve a ingresar.'
        : '';
  ...
  protected submit(): void {
    ...
    request.subscribe({
      next: (user) => void this.router.navigate([
        user.debeCambiarPassword || user.debeCambiarPin ? '/mi-acceso' : '/dashboard'
      ]),
      error: (requestError) => {
